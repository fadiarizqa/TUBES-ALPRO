...
def read_data_sewa(): 
    conn_0309 = sqlite3.connect('sewa_gedung.db')
    cur_0309 = conn_0309.cursor()
    cur_0309.execute("SELECT * FROM data_penyewa")
    result_0309 = cur_0309.fetchall()
    conn_0309.commit()
    conn_0309.close()
    return result_0309

def read_data_gedung():
    conn_0309 = sqlite3.connect('sewa_gedung.db')
    cur_0309 = conn_0309.cursor()
...
def search_status_sewa(ID_Penyewa_0309 : int):
    conn_0309 = sqlite3.connect('sewa_gedung.db')
    cur_0309 = conn_0309.cursor()
    cur_0309.execute("SELECT ID_Penyewa, nama_penyewa, ID_Gedung, nama_gedung, tanggal, jam, nominal_pembayaran, status FROM data_penyewa WHERE ID_Penyewa = ?", (ID_Penyewa_0309,))
    result_0309 = cur_0309.fetchall()
    tabel_0309= PrettyTable()
    tabel_0309.field_names = ["ID Penyewa", "Nama", "ID Gedung", "Nama Gedung", "Tanggal", "Jam", "Nominal Pembayaran", "Status"]
...
def visualisasi_data(x_col_0309, y_col_0309, chart_type_0309):
    ...
    if data_0309:
        df = pd.DataFrame(data_0309, columns=[x_col_0309, y_col_0309])
        plt.figure(figsize=(10, 6))
        
        if chart_type_0309 == 'bar':
            sns.barplot(data=df, x=x_col_0309, y=y_col_0309)
        elif chart_type_0309 == 'line':
            sns.lineplot(data=df, x=x_col_0309, y=y_col_0309)
        elif chart_type_0309 == 'scatter':
            sns.scatterplot(data=df, x=x_col_0309, y=y_col_0309)
        else:
